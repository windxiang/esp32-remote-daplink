...
uint32_t DAP_ProcessVendorCommand(const uint8_t *request, uint8_t *response) {
  ...
  *response++ = *request;        // copy Command ID
  ...
  switch (*request++) {          // first byte in request is Command ID
    ...
    case ID_DAP_Vendor8: {
        printf("ID_DAP_Vendor8\r\n");
        *response = 1;
        if (0 == *request) {
            // main_usb_set_test_mode(false); // TODO 先注释
        } else if (1 == *request) {
            // main_usb_set_test_mode(true); // TODO 先注释
        } else {
            *response = 0;
        }

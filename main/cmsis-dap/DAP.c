...
#else
        // 获取Serial Number
        strcpy((char*)info, "070000014748f01f0334343835373948");
        length = strlen((char*)info) + 1;
    // TODO 先注释
...
static TickType_t TimerTick;

// Start Timer
static __inline void TIMER_START (uint32_t usec) {
    // 修改 使用RTOS的方式
    TimerTick = xTaskGetTickCount() + (usec / 1000); // 微秒转换为毫秒
//   SysTick->VAL  = 0U;
...
//   return ((SysTick->CTRL & SysTick_CTRL_COUNTFLAG_Msk) ? 1U : 0U); // 修改 使用RTOS的方式
return 0;
}
